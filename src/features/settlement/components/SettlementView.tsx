...
export function SettlementView() {
    ...
    const notGeneratedYet = error
        ? normalizeApiError(error).status === 404
        : false;

    if (error && !notGeneratedYet) {
        return (
            <ErrorState error={error} title="Couldn't load the settlement" />
        );
    }
                    ...
                    {data.status === "RECALCULATING" ? (
                        <Card className="border-(--warning)/40 bg-(--warning)/10">
                            <CardContent className="pt-5 text-sm">
                                This cycle was reopened, so these figures are
                                out of date. Regenerate the settlement once the
                                corrections are in.
                            </CardContent>
                        </Card>
                    ) : null}
                    ...
                    {data.status === "FINALIZED" ? (
                        <Card className="border-(--success)/40 bg-(--success)/10">
                            <CardContent className="flex items-center gap-2 pt-5 text-sm">
                                ...
                                This settlement is finalized. The figures are
                                locked.
                            </CardContent>
                        </Card>
                    ) : null}
