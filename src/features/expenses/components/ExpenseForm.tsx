...
export function ExpenseForm({ type, expense, redirectTo }: ExpenseFormProps) {
    ...
    useEffect(() => {
        if (
            ...
            (form.getValues("allocations") ?? []).length === 0
        ) {
    ...
    const onSubmit = async (values: ExpenseFormValues) => {
        const included = (values.allocations ?? []).filter(
            (allocation) => allocation.included,
        );
        ...
        /> */}
                ...
                {!expense && !canReviewExpenses ? (
                    <p className="text-sm text-muted-foreground">
                        A member or manager will review this before it counts
                ...
                {isClosed ? (
                    <p className="text-sm text-muted-foreground">
                        This cycle is closed, so expenses can&apos;t be added or
                        edited.
                    </p>
                ) : null}
            </form>
