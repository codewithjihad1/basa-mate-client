...
¹ `MEMBER` can create expenses and deposits, but every record they create starts as `PENDING` and
only counts toward the settlement once an OWNER or MANAGER approves it (see
[Expense approval workflow](#expense-approval-workflow) and
[Deposit approval workflow](#deposit-approval-workflow)). `VIEWER` cannot write at all.
...
```json
{
    ...
    "error": {
        ...
        "details": [{ "path": "name", "message": "Basa name must be at least 2 characters" }]
    }
}
```
...
| `INVALID_DEPOSIT`              | 404       | Deposit not found (or soft-deleted)                                      |
| `EXPENSE_ALREADY_REVIEWED`     | 400       | Expense is not `PENDING`, so it cannot be approved or rejected           |
| `DEPOSIT_ALREADY_REVIEWED`     | 400       | Deposit is not `PENDING`, so it cannot be approved or rejected           |
...
| `CYCLE_HAS_PENDING_APPROVALS`  | 409       | Cycle close refused while pending approvals exist                        |
...
Errors: `401 INVALID_CREDENTIALS` (wrong email _or_ password — deliberately indistinguishable),
...
```json
{
    ...
    "members": [
        {
            "id": "cmtjmjzyo0002hjcj4nn6p0s1",
            "userId": "cmtjmjzxb0000hjcja9ffvjkw",
            "role": "OWNER",
            "status": "ACTIVE",
    ...
    "mealTypes": [{ "id": "cmtjmjzyw0003hjcj1d7x42cj", "name": "Breakfast", "isActive": true, "sortOrder": 1 }],
    "expenseCategories": [{ "id": "cmtjmjzz20006hjcjcmeaxf1y", "name": "Grocery", "isActive": true, "sortOrder": 1 }]
}
```
    ...
    "items": [
        {
            ...
            "endDate": "2026-09-30T17:59:59.999Z",
            "status": "ACTIVE",
            "_count": { "meals": 0, "expenses": 0, "deposits": 0 }
        }
    ],
...
**Requires:** any active member. Cycle totals for a summary screen. Totals count only
`APPROVED` expenses and deposits (see [Approval workflow](#expense-approval-workflow)).
...
Closing is refused with `409 CYCLE_HAS_PENDING_APPROVALS` while any expense or deposit in the cycle
is still `PENDING` — approve or reject them first, so nothing is silently excluded from the
settlement.
...
**Requires:** any active member except `VIEWER`. A `MEMBER` creates the expense in `PENDING` status;
OWNER and MANAGER create it immediately `APPROVED` (see
...
```json
{
    ...
    "reviewedBy": "cmtjmjzxb0000hjcja9ffvjkw",
    "reviewedAt": "2026-09-02T04:57:20.197Z",
    "allocations": [
        {
            "id": "cmtjmk04w000lhjcjhhetddhs",
            "expenseId": "cmtjmk04v000khjcj1l7szqgr",
            "memberId": "cmtjmjzyo0002hjcj4nn6p0s1",
            "amount": "500",
            "percentage": null
        }
...
When created by a `MEMBER`, the same shape comes back with `"approvalStatus": "PENDING"` and
`reviewedBy`/`reviewedAt` both `null` until a manager or owner reviews it.
...
| `status`                 | enum     | `PENDING` \| `APPROVED` \| `REJECTED` — filter by approval status |
...
`allocations[].member.user` and the approval fields (`approvalStatus`, `reviewedBy`, `reviewedAt`).
Soft-deleted expenses are excluded. Managers/owners typically call with `status=PENDING` to build a
review queue.
...
**Requires:** OWNER or MANAGER, **or** the `MEMBER` who created the expense while it is still
`PENDING`. Every create field is optional; omitted fields are untouched.
...
**Requires:** OWNER or MANAGER, **or** the `MEMBER` who created the expense while it is still
`PENDING`. Soft delete (`softDeleted: true`, `deletedAt` stamped); the row stops counting toward
settlements and listings. Blocked on a closed cycle.

```json
{ "success": true, "data": { "id": "cmtjmmy2n00130acjfm042va2" }, "message": "Expense deleted" }
```

#### `POST /…/expenses/:expenseId/approve` · `POST /…/expenses/:expenseId/reject`

**Requires:** OWNER or MANAGER.

Reviews a `PENDING` expense. `/approve` sets `approvalStatus` to `APPROVED`, `/reject` sets it to
`REJECTED`; both stamp `reviewedBy` and `reviewedAt` with the reviewer, write a
`EXPENSE_APPROVED` / `EXPENSE_REJECTED` audit log entry, and notify the member who created the
expense (unless they reviewed their own record). The response is the updated expense.

Errors: `404 INVALID_EXPENSE`, `400 CYCLE_CLOSED`, `400 EXPENSE_ALREADY_REVIEWED`,
`403 INSUFFICIENT_PERMISSION`.

#### Expense approval workflow

1. A `MEMBER` records an expense (`POST /…/expenses`) → `approvalStatus: "PENDING"`.
2. OWNER/MANAGER review it via `/approve` or `/reject`.
3. Only `APPROVED` expenses feed the [settlement](#settlement-calculation), `GET …/report` and the
   [cycle dashboard](#get-basas-basaid-cycles-cycleid-dashboard). `PENDING` and `REJECTED` ones are
   visible in listings (filter with `status`) but count toward nothing.
4. A member may edit or delete their own expense only while it is still `PENDING`; once reviewed it
   can only be changed by an OWNER or MANAGER.
5. A cycle with any `PENDING` expense cannot be closed.
...
**Requires:** any active member except `VIEWER`. A `MEMBER` records the deposit in `PENDING` status;
OWNER and MANAGER record it immediately `APPROVED` (see
[Deposit approval workflow](#deposit-approval-workflow)).
...
```json
{
    ...
    "approvalStatus": "APPROVED",
    "reviewedBy": "cmtjmjzxb0000hjcja9ffvjkw",
    "reviewedAt": "2026-09-02T04:57:20.085Z",
    "softDeleted": false,
    "member": { "id": "cmtjmjzyo0002hjcj4nn6p0s1", "role": "OWNER", "user": { "name": "Doc User" } }
}
```

When recorded by a `MEMBER`, `approvalStatus` is `"PENDING"` and `reviewedBy`/`reviewedAt` are `null`
until a manager or owner reviews it.
...
**Requires:** any active member. Query: `status` (`PENDING` \| `APPROVED` \| `REJECTED`, optional).
All non-deleted deposits in the cycle, newest `transactionDate` first, each with `member.user` and the
approval fields. Not paginated. Managers/owners typically call with `status=PENDING` to build a
review queue.

#### `PATCH /…/deposits/:depositId`

**Requires:** OWNER or MANAGER, **or** the `MEMBER` who recorded the deposit while it is still
`PENDING`. All create fields optional. Blocked on a closed cycle.

#### `DELETE /…/deposits/:depositId`

**Requires:** OWNER or MANAGER, **or** the `MEMBER` who recorded the deposit while it is still
`PENDING`. Soft delete. Returns `{ "id": "…" }`.

#### `POST /…/deposits/:depositId/approve` · `POST /…/deposits/:depositId/reject`

**Requires:** OWNER or MANAGER.

Reviews a `PENDING` deposit. `/approve` sets `approvalStatus` to `APPROVED`, `/reject` sets it to
`REJECTED`; both stamp `reviewedBy`/`reviewedAt`, write a `DEPOSIT_APPROVED` / `DEPOSIT_REJECTED`
audit log entry, and notify the member who recorded the deposit (unless they reviewed their own
record). The response is the updated deposit.

Errors: `404 INVALID_DEPOSIT`, `400 CYCLE_CLOSED`, `400 DEPOSIT_ALREADY_REVIEWED`,
`403 INSUFFICIENT_PERMISSION`.

#### Deposit approval workflow

1. A `MEMBER` records a deposit (`POST /…/deposits`) → `approvalStatus: "PENDING"`.
2. OWNER/MANAGER review it via `/approve` or `/reject`.
3. Only `APPROVED` deposits feed the [settlement](#settlement-calculation), `GET …/report` and the
   [cycle dashboard](#get-basas-basaid-cycles-cycleid-dashboard). `PENDING` and `REJECTED` ones are
   visible in listings (filter with `status`) but count toward nothing.
4. A member may edit or delete their own deposit only while it is still `PENDING`; once reviewed it
   can only be changed by an OWNER or MANAGER.
5. A cycle with any `PENDING` deposit cannot be closed.
    ...
    "items": [
        {
            ...
            "member": {
                "id": "cmtjmjzyo0002hjcj4nn6p0s1",
                "role": "OWNER",
                "user": { "id": "cmtjmjzxb0000hjcja9ffvjkw", "name": "Doc User" }
            }
        }
    ]
}
...
Full cycle export as JSON: the cycle, its settlement (or `null`), every **approved** expense with its
category, meal totals per member and **approved** deposit totals per member. `PENDING` and `REJECTED`
expenses/deposits are excluded.
...
The cycle's **approved** expenses as CSV — `Date, Description, Category, Amount`, all fields quoted.
...
Review actions notify the affected member: approving/rejecting an expense sends an `EXPENSE`
notification titled "Expense approved" / "Expense rejected"; approving/rejecting a deposit sends a
`SYSTEM` notification titled "Deposit approved" / "Deposit rejected". The reviewer is not notified
...
- **Grocery** — `Expense` rows with `type = GROCERY`, `approvalStatus = APPROVED` and
  `softDeleted = false`.
- **Shared** — `ExpenseAllocation` rows belonging to `type = SHARED`, `approvalStatus = APPROVED`,
  non-deleted expenses.
- **Deposits** — `DepositTransaction` rows with `approvalStatus = APPROVED` and
  `softDeleted = false`.
- **Members** — memberships with `status = ACTIVE` at generation time.

`OTHER` expenses are excluded from the settlement entirely, and `PENDING`/`REJECTED` expenses and
deposits never feed the calculation — approvals must be resolved before the cycle can be closed
(see [close](#post-basas-basaid-cycles-cycleid-close)).
...
1. **`GET /settlements/:settlementId` and `/items` are unusable.**
...
2. **`memberId` means different things in different endpoints.** `POST /meals` expects a **user id**;
...
3. **`SettlementService.reopen` has no route.** Settlements are only moved to `RECALCULATING`
   indirectly, by reopening the cycle.

4. **`reopen` discards its `reason`.** The field is required and validated, then dropped — no audit
   record is written.

5. **Email verification is never enforced.** `requireVerifiedEmail` exists but is not mounted on any
   route, and verification emails are only sent when `EMAIL_PROVIDER != "console"` or in production.

6. **Rate limiting is per-process and in-memory.** It resets on restart and does not hold across
